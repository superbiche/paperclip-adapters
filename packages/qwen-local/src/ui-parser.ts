/**
 * Self-contained UI parser for qwen-local adapter.
 *
 * Parses Qwen Code CLI stream-json stdout into structured transcript entries
 * for the Paperclip run viewer. Zero runtime imports — eval'd in a sandboxed worker.
 *
 * Ported from the in-tree packages/adapters/qwen-local/src/ui/parse-stdout.ts
 * with all shared-utils helpers inlined to remove runtime dependencies.
 */
...
function readUsage(parsed: Record<string, unknown>): {
  ...
  const usageMetadata = usage ? asRecord(usage.usageMetadata) : null;
