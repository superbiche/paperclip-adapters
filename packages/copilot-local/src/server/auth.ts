 ...
 *
 * Ported from paperclipai/paperclip#3629 (HearthCore). `fetchWithRetry` is
 * sourced from this package's vendored copy until upstream `adapter-utils`
 * publishes it.
 */
...
// Keyed by token fingerprint + host to avoid cross-tenant cache poisoning in
...
/** Test-only — clear the endpoint discovery cache between assertions. */
